...
class BuscarPersonalInformes extends StatefulWidget {
  const BuscarPersonalInformes({Key? key}) : super(key: key);

  @override
  State<BuscarPersonalInformes> createState() => _BuscarPersonalInformesState();
}

class _BuscarPersonalInformesState extends State<BuscarPersonalInformes> {
                ...
                child: SizedBox(
                    ...
                    child: Consumer<InformeController>(
                      builder: (_, provider, __) {
                        if (provider.getErrorInfoGuardiaPorCliente == null) {
                          return const NoData(
                            label: 'Ingrese dato para búsqueda',
                          );
                        } else if (provider.getErrorInfoGuardiaPorCliente ==
                            false) {
                          return const NoData(
                            label: 'No existen datos para mostar',
                          );
                        } else if (provider
                            .getListaInfoGuardiaPorCliente.isEmpty) {
                          return const NoData(
                            label: 'No existen datos para mostar',
                          );
                          // Text("sin datos");
                        ...
                        return Consumer<InformeController>(
                          builder: (_, values, __) {
                            return ListView.builder(
                              itemCount:
                                  values.getListaInfoGuardiaPorCliente.length,
                              itemBuilder: (BuildContext context, int index) {
                                final guardia =
                                    values.getListaInfoGuardiaPorCliente[index];
                                return Card(
                                  child: ListTile(
                                    ...
                                    onTap: () {
                                      values.setGuardiaInforme(guardia);
                                    },
                                  ),
                                );
                              },
