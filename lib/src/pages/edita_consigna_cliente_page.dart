...
// import 'package:provider/provider.dart';
// import 'package:sincop_app/src/controllers/consignas_clientes_controller.dart';
// import 'package:sincop_app/src/controllers/consignas_controller.dart';
// import 'package:sincop_app/src/utils/responsive.dart';
// import 'package:sincop_app/src/utils/theme.dart';

// class EditarConsignaCliente extends StatefulWidget {
//   const EditarConsignaCliente({Key? key}) : super(key: key);

//   @override
//   State<EditarConsignaCliente> createState() => _EditarConsignaClienteState();
// }

// class _EditarConsignaClienteState extends State<EditarConsignaCliente> {
//   TextEditingController _fechaInicioController = TextEditingController();
//   TextEditingController _fechaFinController = TextEditingController();
//   TextEditingController _horaInicioController = TextEditingController();
//   TextEditingController _horaFinController = TextEditingController();
//   final consignaController = ConsignasClientesController();
...
//                 ),
//                 TextFormField(
//                   initialValue: consignaController.getAsunto,
//                   decoration: const InputDecoration(
//                       // suffixIcon: Icon(Icons.beenhere_outlined),
//                       ),
//                   textAlign: TextAlign.start,
...
//                 ),
//                 TextFormField(
//                   initialValue: consignaController.getDetalle,
//                   maxLines: 2,
//                   decoration: const InputDecoration(
//                       // suffixIcon: Icon(Icons.article_outlined),
//                       ),
...
//   // }
//   void _seleccionaHoraInicio(
//       context, ConsignasController consignaController) async {
//     TimeOfDay? _hora =
//         await showTimePicker(context: context, initialTime: timeInicio);
...
import 'package:nseguridad/src/pages/view_photo_consignas_cliente.dart';
...
class EditarConsignaClientePage extends StatefulWidget {
  const EditarConsignaClientePage({Key? key}) : super(key: key);
...
class _EditarConsignaClientePageState extends State<EditarConsignaClientePage> {
  ...
  Widget build(BuildContext context) {
    final Responsive size = Responsive.of(context);
    final consignaController = Provider.of<ConsignasController>(context);
        ...
        child: Scaffold(
         ...
         flexibleSpace: Container(
            decoration: BoxDecoration(
              ...
              title:  Text('Editar Consigna',
              // style:  Theme.of(context).textTheme.headline2,
              ),
              actions: [
                Consumer<SocketService>(
                  builder: (_, valueSave, __) {
                    return valueSave.serverStatus == ServerStatus.Online
                        ? Container(
                            margin: EdgeInsets.only(right: size.iScreen(1.5)),
                            child: IconButton(
                                splashRadius: 28,
            ...
            body: Consumer<SocketService>(
              builder: (_, valueSocket, __) {
                return valueSocket.serverStatus == ServerStatus.Online
                    ? Container(
                        // color: Colors.red,
                        margin: EdgeInsets.only(top: size.iScreen(0.0)),
                        padding: EdgeInsets.only(
                          ...
                          child: Form(
                            ...
                            child: Column(
                              children: [
                                ...
                                TextFormField(
                                  // controller:consignaController.getAsunto ,
                                  initialValue: consignaController.getAsunto,
                                  // decoration: const InputDecoration(
                                  //     suffixIcon: Icon(Icons.beenhere_outlined)),
                                  textAlign: TextAlign.start,
                                ...
                                ElevatedButton(
                                  onPressed: () =>
                                      multiSelectPicker(consignaController),
                                ...
                                Consumer<ConsignasController>(
                                  builder: (_, valorFecha, __) {
                                    return valorFecha.getLitaMultiSelect != null
                                        ? 
                                        Wrap(
                                            alignment:
                                                ...
                                                valorFecha
                                                    ...
                                                    .map(
                                                      (e) =>
                                                          GestureDetector(
                                                            onTap: ()  {
                                                              valorFecha.eliminaFechaConsigna(e);
                                                              // print(' valorFecha.eliminaFechaConsigna(e)');
                                                            },
                                                            child: Container(
                                                              margin: EdgeInsets
                                                                  .all(size
...
//*****************************************/
  ...
  //================================================= NUEVO PICKER==================================================//
  Future<void> multiSelectPicker(ConsignasController consignaController) async {
    final List<DateTime>? picked = await showDialog<List<DateTime>>(
      ...
      builder: (BuildContext context) {
        ...
        return AwesomeCalendarDialog(
          selectedDates: consignaController.getLitaMultiSelect,
    ...
    if (picked != null) {
      // print('picked: $picked');
      consignaController.setFechasConsigna(picked);
      //   multiSelect = picked;
      // setState(() {
      // });
    }
...
//********************************************************************************************************************//
  void _onSubmit(BuildContext context, ConsignasController controller) async {
    final isValid = controller.validateForm();
    if (!isValid) return;
    if (isValid) {
      if (controller.getListaGuardiasConsigna.isEmpty) {
        NotificatiosnService.showSnackBarDanger('Debe asignar el personal');
      } else if (controller.getLitaSeleccionada == null ||
          controller.getLitaSeleccionada!.isEmpty) {
        NotificatiosnService.showSnackBarDanger('Debe elegir fecha');
      } else {
        final conexion = await Connectivity().checkConnectivity();
        if (conexion == ConnectivityResult.none) {
          NotificatiosnService.showSnackBarError('SIN CONEXION A INTERNET');
  ...
  void bottomSheet(
    ConsignasController controllerConsinas,
    BuildContext context,
    Responsive size,
  ) {
    showCupertinoModalPopup(
  ...
  void _funcionCamara(
      ImageSource source, ConsignasController consignasController) async {
    final picker = ImagePicker();
    final XFile? pickedFile = await picker.pickImage(
      source: source,
      imageQuality: 100,
...
class _ItemFoto extends StatelessWidget {
  final CreaNuevaFotoConsignaCliente? image;
  final ConsignasController controllerConsinas;
      ...
      child: Consumer<ConsignasController>(
        builder: (_, provider, __) {
          return Container(
              ...
              width: size.wScreen(100.0),
              height: size.iScreen(
                  provider.getListaGuardiasConsigna.length.toDouble() * 6),
              child: Wrap(
                children: provider.getListaGuardiasConsigna
                    .map((e) => Card(
                          color: Colors.grey.shade200,
                          child: InkWell(
                            onTap: (() =>
                                provider.eliminaGuardiaConsigna(e['id'])),
                            child: Row(
                              children: [
                                // Container(
                                //   // color: Colors.green,
                                //   margin: EdgeInsets.only(right: size.iScreen(0.0)),
                                //   padding: EdgeInsets.all( size.iScreen(0.5)),
                                //   child:
                                //   //  Text('X', style: GoogleFonts.lexendDeca(
                                //   //       fontSize: size.iScreen(2.0),
                                //   //       color: Colors.red,
                                //   //       fontWeight: FontWeight.normal),),
                                //   // Icon(Icons.delete_forever_outlined,
                                //   //     color: Colors.red,size: size.iScreen(3.5),),
                                // ),
                                Expanded(
                                  child: Container(
                                    margin: EdgeInsets.symmetric(
